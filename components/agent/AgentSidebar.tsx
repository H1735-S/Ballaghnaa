...
import { LayoutDashboard, FileText, User, ChevronLeft, BarChart2, History, ListOrdered, Tag } from "lucide-react";
  ...
  return (
    <Tooltip label={label} show={collapsed}>
      <Link href={href} onClick={onClose} className={cn(
        ...
        {isActive && <motion.span layoutId="agent-active-bg" className="absolute inset-0 rounded-xl bg-gradient-to-r from-primary to-primary/80" transition={{ type: "spring", stiffness: 380, damping: 30 }} />}
            ...
            <motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "auto" }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.2, ease: "easeInOut" }}
              ...
              {badge !== undefined && (
                <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }}
                  className={cn("ms-auto flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-xs font-bold", isActive ? "bg-white/25 text-white" : "bg-primary/15 text-primary")}>
                  {badge}
                </motion.span>
              )}
            </motion.span>
...
export function AgentSidebar({ collapsed, onToggle, onClose }: { collapsed: boolean; onToggle: () => void; onClose?: () => void }) {
  const pathname = usePathname();
  const { t } = useLang();
  const { trigger } = usePageTransition();

  const navItems = [
    { href: "/agent",                    icon: LayoutDashboard, label: t("الرئيسية",        "Dashboard")          },
    { href: "/agent/complaints",         icon: FileText,        label: t("الشكاوى",          "Complaints")         },
    { href: "/agent/queue",              icon: ListOrdered,     label: t("قائمة الانتظار",   "Queue")              },
    { href: "/agent/stats",              icon: BarChart2,       label: t("إحصائياتي",        "My Stats")           },
    { href: "/agent/history",            icon: History,         label: t("السجل",            "History")            },
    { href: "/agent/category-request",   icon: Tag,             label: t("طلب فئة جديدة",   "Request Category")   },
    { href: "/agent/profile",            icon: User,            label: t("ملفي الشخصي",      "My Profile")         },
  ];
  ...
  return (
    <motion.aside animate={{ width: collapsed ? 72 : 260 }} transition={{ type: "spring", stiffness: 320, damping: 32 }}
      ...
      <button onClick={() => trigger('/agent')} className={cn("relative flex h-20 shrink-0 items-center border-b border-sidebar-border transition-colors hover:bg-accent/40 w-full cursor-pointer bg-transparent p-0", collapsed ? "justify-center px-3" : "gap-3 px-5")}>
        ...
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }} className="min-w-0 overflow-hidden">
              ...
              <p className="truncate text-xs leading-tight text-muted-foreground">{t("بوابة الوكيل","Agent Portal")}</p>
            </motion.div>
          )}
        </AnimatePresence>
      </button>
      ...
      <nav className={cn("flex-1 overflow-y-auto py-4", collapsed ? "px-2" : "px-3")}>
        ...
        <div className="space-y-1">
          {navItems.map((item, i) => (
            <motion.div key={item.href} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04, duration: 0.25 }}>
              <SidebarItem {...item} isActive={pathname === item.href || (item.href !== "/agent" && pathname.startsWith(item.href))} collapsed={collapsed} onClose={onClose} />
            </motion.div>
          ))}
        </div>
      </nav>
      ...
      <div className={cn("shrink-0 border-t border-sidebar-border py-3", collapsed ? "flex justify-center px-2" : "px-3")}>
        <Tooltip label={collapsed ? t("توسيع","Expand") : ""} show={collapsed}>
          <motion.button onClick={onToggle} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            className={cn("flex items-center gap-2 rounded-xl border border-border/60 bg-muted/40 text-xs font-medium text-muted-foreground hover:bg-accent hover:text-foreground transition-all duration-200", collapsed ? "w-10 h-10 justify-center p-0" : "w-full px-3 py-2")}
