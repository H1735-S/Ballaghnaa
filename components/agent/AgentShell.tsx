...
export function AgentShell({ user, children }: Props) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      <AnimatePresence>
        {!isLarge && mobileOpen && (
          <motion.div className="fixed inset-0 z-20 bg-black/50 backdrop-blur-sm lg:hidden"
      ...
      {isLarge === null ? (
        ...
        <div className="relative z-10 flex h-full flex-col">
          ...
          <motion.button onClick={() => setCollapsed(v => !v)}
            ...
            className="absolute end-0 top-1/2 z-50 flex h-7 w-7 -translate-y-1/2 translate-x-1/2 items-center justify-center rounded-full border border-border bg-background shadow-md hover:bg-accent hover:shadow-lg transition-all duration-200"
        ...
        <motion.div className="fixed inset-y-0 start-0 z-30 flex flex-col"
          ...
          <AgentSidebar collapsed={false} onToggle={() => setMobileOpen(false)} onClose={() => setMobileOpen(false)} />
        </motion.div>
      )}

      <div className="flex flex-1 flex-col overflow-hidden">
        <AgentNavbar user={user} onMenuToggle={() => setMobileOpen(v => !v)} />
