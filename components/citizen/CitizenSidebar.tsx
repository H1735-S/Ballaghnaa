...
import {
  LayoutDashboard, FileText, PlusCircle, User, BarChart2,
} from "lucide-react";
...
function SidebarItem({
  href, icon: Icon, label, badge, isActive, collapsed, onClose,
}: {
  ...
  return (
    <Tooltip label={label} show={collapsed}>
      ...
      >
        ...
        {!isActive && (
          <span className="absolute inset-0 rounded-xl bg-transparent transition-colors duration-200 group-hover:bg-accent" />
        )}
            ...
            >
              ...
              {badge !== undefined && (
                <motion.span
                  ...
                  animate={{ scale: 1 }}
                  className={cn(
                    "ms-auto flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-xs font-bold",
        ...
        {collapsed && badge !== undefined && (
          <span className="absolute end-1 top-1 h-2 w-2 rounded-full bg-primary ring-2 ring-sidebar" />
        )}
      </Link>
    </Tooltip>
  );
...
export function CitizenSidebar({
  ...
  const navItems = [
    { href: "/citizen",                icon: LayoutDashboard, label: t("الرئيسية",     "Dashboard")     },
    { href: "/citizen/complaints",     icon: FileText,        label: t("شكاواي",       "My Complaints") },
    { href: "/citizen/complaints/new", icon: PlusCircle,  label: t("شكوى جديدة",   "New Complaint") },
    { href: "/citizen/stats",          icon: BarChart2,   label: t("إحصائياتي",    "My Stats")      },
    { href: "/citizen/profile",        icon: User,        label: t("الملف الشخصي", "Profile")       },
  ];
          ...
          {!collapsed && (
            ...
            >
              ...
              <p className="truncate text-xs leading-tight text-muted-foreground">
                {t("بوابة المواطن", "Citizen Portal")}
              </p>
            </motion.div>
          )}
        </AnimatePresence>
